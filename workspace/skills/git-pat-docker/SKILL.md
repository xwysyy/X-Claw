---
name: git-pat-docker
description: Configure and troubleshoot Git HTTPS PAT auth inside PicoClaw Docker containers (`gateway`/`agent`), including `tools.git`, credential helper checks, clone/push workflow, and safety-guard workarounds.
---

# Git PAT in Docker

Use this skill when:
- You need `git clone/pull/push` from inside PicoClaw containers.
...
- Runtime config file inside container: `/home/picoclaw/.picoclaw/config.json`
...
```bash
source ~/.zshrc && proxy_on && \
docker compose -p picoclaw -f docker/docker-compose.yml --profile agent build picoclaw-agent
```
...
```bash
docker exec picoclaw-gateway sh -lc '
...
Expected:
- `credential.helper` contains `store --file=/home/picoclaw/.git-credentials`
...
```bash
docker exec picoclaw-gateway sh -lc '
cd "$HOME/.picoclaw/workspace/<repo>" &&
git add -A &&
git commit -m "update" &&
