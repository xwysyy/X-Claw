# X-Claw Roadmap（集百家之长 / 差异化特性）

这是一份**可持续维护**的路线图：目标不是“功能堆满”，而是把 X-Claw 打造成在 **pico-scale（轻量、随处部署）** 前提下，具备 **可靠性 / 可解释性 / 可复现性 / 生态可扩展性** 的个人 AI 助手。
...
当前待分析的 `ref/` 项目清单（按“对 X-Claw 可落地价值”排序）：

- [x] `ref/nagobot`（Go，轻量 personal assistant）— 2026-03-01
- [x] `ref/clawlet`（Go，SQLite + sqlite-vec 混合记忆）— 2026-03-02
- [x] `ref/nanobot`（Python，~4k 核心 + MCP）— 2026-03-02
- [x] `ref/blades`（Go，tool schema + middleware + flow）— 2026-03-02
- [x] `ref/trpc-agent-go`（Go，LangGraph-like + checkpoint + MCP）— 2026-03-02
- [x] `ref/openai-swarm`（Python，极简 multi-agent handoff）— 2026-03-02
- [x] `ref/langgraph`（Python，durable execution / checkpoint / interrupt）— 2026-03-02
- [x] `ref/mem0`（Python，scope 化记忆 API：user/agent/run）— 2026-03-02
- [x] `ref/letta`（Python，MemGPT：memory blocks + 只读约束）— 2026-03-02
- [x] `ref/chroma`（Python，向量库/服务化；作为取舍对照）— 2026-03-02
...
- [x] `ref/nanoclaw`（TS，容器隔离的轻量 assistant：group queue + scheduler + SQLite）— 2026-03-02
- [x] `ref/zeroclaw`（Rust，pico-scale 竞品：MCP 多传输 + estop + runtime trace）— 2026-03-02

---

## 1) 北极星：我们要做出的“独特 feature”

对标 claw 类竞品时，X-Claw 的差异化不应该是“更多按钮”，而是：**把真实长期使用的痛点做成一等公民能力**。
...
4) **MCP Bridge（生态护城河）**  
   不靠维护 1000+ 内置工具取胜，而是用 MCP 标准桥接外部工具生态（企业内/社区都可复用），并套上 X-Claw 的策略层。
...
- C1：workspace 级 SQLite FTS5 索引（无需向量也能明显提升可用性）
...
> 说明：这里的目标不是写“项目介绍”，而是写：**它解决了什么痛点 → 关键实现在哪里 → X-Claw 如何落地（文件落点/PR 切法/风险护栏）**。
...
这类能力非常适合 X-Claw 的定位：不追求框架化，而追求**长期跑得住**。
...
- Tool 审计日志（每 call 一份 md/json）
  - X-Claw 落点候选：`pkg/tools/toolcall_executor.go`
  - 设计建议：
    - 默认关闭，`config.tools.audit.enabled=true` 时启用
    - 目录建议跟 workspace 走：`<workspace>/.x-claw/audit/tools/`
    - 同时写 `*.md`（人读）+ `*.json`（未来做 replay/统计）

- Thread Manager（并发会话调度）
  - X-Claw 已有 `channels.Manager` + bus，建议不要照搬 thread 架构
  - 借鉴点：
...
#### (4) 能直接转化成 X-Claw 独特 feature 的点

把 nagobot 的工程点组合起来，X-Claw 可以对外说一句很硬的卖点：
...
### 3.2 ✅ clawlet（Go）— “本地混合记忆检索（FTS+向量）+ 安全默认值” 的范本（已完成：2026-03-02）
...
这非常贴合 X-Claw 的路线：我们不需要引入重型向量服务，也能把记忆与安全做得更“可用且可控”。
...
#### (3) 对 X-Claw 的落地映射（按 Phase 对齐）

- 对齐 Phase B（结构化记忆）：
  - 先做 **B1：`memory_search`/`memory_get` 输出升级为 JSON hits**  
    - X-Claw 落点：`pkg/agent/memory_tool.go`
...
### 3.3 ✅ nanobot（Python）— “MCP 桥接 + 参数校验 + 运行时元信息防注入” 的轻量实现（已完成：2026-03-02）
...
对 X-Claw 来说，nanobot 更像一套“把工具生态接入做对”的参考实现，而不是要抄它的语言/框架。
...
### 3.4 ✅ blades（Go）— “Tool Middleware + Flow 组合 + 可恢复 Runner” 的组件库思路（已完成：2026-03-02）
...
对 X-Claw 来说，最值钱的是：它提供了一套把“策略层/确认/重试/审计”从工具实现里剥离出来的范式，非常适合做我们要的 **Policy-first Tools**。
...
- 对齐 Phase A（可观测/可复现）+ Phase E（durable execution）：
  - blades 的 “InvocationID + 去重” 思路可用于 X-Claw 的 checkpoint/resume：  
...
- 对齐 Phase F（多 agent）：
  - blades 的 Routing/flow 是“可借鉴但不照搬”的：X-Claw 可保持轻量，只抄 handoff/路由提示套路
...
### 3.5 ✅ trpc-agent-go（Go）— “工程级 MCP ToolSet + SQLite CheckpointSaver” 的参考实现（已完成：2026-03-02）
...
对 X-Claw 来说，价值在“实现方式”，不在“整套 GraphAgent 框架”。
...
- 对齐 Phase D（MCP Bridge）：
  - X-Claw 的 MCP 最小版本建议直接对齐 trpc-agent-go 的关键工程点：
...
### 3.6 ✅ openai-swarm（Python）— “handoff 作为一等工具结果” 的极简原语（已完成：2026-03-02）
...
对 X-Claw 来说，这能把“多 agent 协作”从“隐式路由”升级为“显式、可回放、可控”。

#### (2) 最值得抄的 2 个点（够用就好）

1) **Handoff = tool call 的结果里返回另一个 Agent（执行者切换）**  
   - 关键位置：`ref/openai-swarm/swarm/core.py`
   - X-Claw 启发：把 agent 切换做成显式工具 `handoff(agent_name, reason)`，并写入审计/ledger

2) **context_variables 运行时注入但从 tools schema 隐藏**  
   - 关键位置：同上（schema 构建时 pop，执行时注入）
   - X-Claw 启发：runtime context（workspace/channel/chat/sessionKey）只由执行层注入，永远不要让模型“自己传”
...
### 3.7 ✅ langgraph（Python）— durable execution 的“机制拆解”与 SQLite checkpointer（已完成：2026-03-02）
...
X-Claw 不需要引入 LangGraph，但可以直接抄这套拆解方式，用最小代码做出 Phase E 的核心卖点。
...
4) **同步实现明确限制并发（锁保护连接）**  
   ...
   - 启发：X-Claw 的 checkpoint store 也要明确并发模型（按 session 串行最简单）
...
### 3.8 ✅ mem0（Python）— “记忆作用域（user/agent/run）+ 结构化增删改” 的产品化边界（已完成：2026-03-02）
...
### 3.9 ✅ letta（Python）— memory blocks + 只读约束：把“长期记忆”变成可控资产（已完成：2026-03-02）
...
2) **字符数上限（limit）在校验阶段强制执行**  
   ...
   - 启发：X-Claw 的 blocks 也必须有 hard limit。
...
### 3.10 ✅ chroma（Python）— 向量库“服务化路线”的取舍对照（已完成：2026-03-02）

仓库：`ref/chroma`

#### (1) 一句话价值

Chroma 的价值对 X-Claw 来说主要是“边界对照”：当你走到一定规模，本地 sqlite-vec/FTS 可能不够用，这时才需要外部向量服务；但 **把它做成默认依赖会直接破坏 pico-scale**。

所以对 X-Claw 来说：Chroma 不是“要集成的组件”，而是帮助我们明确“什么时候不该自己造轮子”的参照物。
...
### 3.11 ✅ feishu-openclaw（Node）— “无需公网服务器的飞书长连接 + 媒体端到端” 桥接实现（已完成：2026-03-02）
...
这对 X-Claw 的 Phase G（渠道与媒体）是强参考：我们可以把“渠道接入”做成可选的独立 bridge，而不是把所有复杂性都塞进 gateway。
...
2) **消息去重（Feishu 事件可能重复投递）+ TTL cache**  
   - 关键位置：同文件（`seen` + TTL）
   - X-Claw 启发：我们的 Feishu channel 也应加“事件去重层”，避免重复回复

3) **“正在思考…”占位符（阈值触发，后续 update/delete）**  
   - 关键位置：同文件（`THINKING_THRESHOLD_MS`）
   - X-Claw 启发：Channel UX 三件套已经有（typing/placeholder），这个实现细节非常接近生产需求

4) **群聊回复策略：只有 @、问号、动词触发才回应（避免刷屏）**  
   - 关键位置：`shouldRespondInGroup()`  
   - X-Claw 启发：群聊“触发条件”最好做成可配置策略（每个 channel 统一抽象）
...
6) **“MEDIA:” 约定（agent 输出一行 MEDIA: /path 或 URL）→ 桥接自动识别并回传**  
   ...
   - X-Claw 启发：这是跨渠道的好约定，适合把“媒体回传”做成统一协议

7) **本地文件外发白名单（防止误外传敏感文件）**  
   - 关键位置：`ALLOWED_OUTBOUND_MEDIA_DIRS` + `isAllowedOutboundPath()`  
   - X-Claw 启发：任何“从本机读取并发送到外部渠道”的能力都必须先有 allowlist
...
- 对齐 Phase G（渠道与媒体）：
  - 两条路线二选一（都能做，但建议先做更小的）：
    ...
    2) **提供 `x-claw bridge feishu` 独立桥接（长期更稳）**：让 gateway 更轻，bridge 专注渠道复杂性
...
### 3.12 ✅ CoPaw（Python）— “Console + 配置热更新 + 分发安装” 的产品化路径（已完成：2026-03-02）

仓库：`ref/CoPaw`

#### (1) 一句话价值

CoPaw 对 X-Claw 的启发不在算法/agent 框架，而在“产品化工程”：  
...
### 3.13 ✅ open-cowork（TS/Electron）— “桌面端沙箱 + GUI 操作 + Trace Panel + 技能库/MCP” 的产品化参考（已完成：2026-03-02）
...
虽然 X-Claw 不打算走“直接操作用户桌面 GUI”的路线，但 open-cowork 的“产品化方法论”非常值得借鉴：  
...
4) **PermissionDialog：危险工具调用的真实人类确认（allow/deny/always allow）**  
   ...
   - 启发：X-Claw 的 Confirm middleware 可以做到同样体验（CLI 用确认提示，Gateway 用 UI/按钮）
...
6) **MCP tools prompt 的“可解释列表”（告诉模型有哪些 server/tools）**  
   ...
   - 启发：X-Claw 的 MCP Bridge 不仅要“能用”，还要“让模型知道怎么用”（减少失败率）

7) **Skills 作为“带脚本/资源的技能包”交付（但注意 License）**  
   - 关键位置：`ref/open-cowork/.claude/skills/*`  
   - 注意：这些 skill 的 SKILL.md 中有 “proprietary” 标注，**只能借鉴结构与工作流，不应直接拷贝内容到 X-Claw 仓库**。
   - 启发：X-Claw 可以自己做一套“文档产出技能包”（pptx/docx/xlsx/pdf），这是非常强的差异化卖点

#### (3) 对 X-Claw 的落地映射（怎么“借鉴但不变重”）
...
#### (4) 建议从 open-cowork 引出的最小 PR 切法（X-Claw 版本）
...
### 3.14 ✅ pi-mono（TS）— “会话树 JSONL + steering/follow-up + 事件流工具执行” 的交互范式（已完成：2026-03-02）
...
对 X-Claw 的启发很明确：我们不缺工具，缺的是“**可控的交互回路**”和“**可复盘的会话资产**”。
...
1) **Agent 事件流（message/tool 的 start/update/end）作为 UI 基座**  
   ...
   - 启发：X-Claw 的 Tool Trace（Phase A1）不只落盘，也应该能“流式推送”给 CLI/Gateway UI（占位符/进度条/折叠输出）
...
4) **Session = JSONL + 树结构（id/parentId），原地分叉、原地 time-travel**  
   ...
   - 启发：X-Claw 当前 session 更像线性日志；Pi 的树结构让“重试/回滚/走分支方案”变成一等能力，并且完整历史永远保留（LLM 只吃 compact 后的视图）
...
6) **Extensions/Skills/Prompts/Themes 的热加载与事件钩子**  
   ...
   - 启发：X-Claw 的 skills 也可以有“事件点”（before_tool/after_tool/before_compact），让扩展更像 middleware，而不是只提供一堆工具

#### (3) 对 X-Claw 的落地映射（怎么抄到 Go 项目里）
...
### 3.15 ✅ clawdbot-feishu（TS）— 飞书“消息/媒体/策略/工具”全链路工程化样板（已完成：2026-03-02）
...
对 X-Claw 来说，这个仓库是“飞书渠道做深”的最佳参考：**不是 API 能跑通，而是行为稳定 + 可控策略 + 媒体可靠**。

#### (2) 最值得抄的 7 个点（直接对标 X-Claw 的渠道层）
...
4) **流式卡片（streaming_mode）作为“实时输出 UI”**  
   ...
   - 启发：飞书原生支持 streaming card：创建 card → 按 sequence 更新 element → 结束时关 streaming_mode。  
     X-Claw 现在有 placeholder/edit message，但要做“流式输出”时，卡片比反复 edit 普通消息更稳

5) **群策略与 allowlist：open/allowlist/disabled + requireMention + multi-bot group 兼容**  
   - 关键位置：`ref/clawdbot-feishu/src/policy.ts`
   - 启发：X-Claw 已有 `allow_from`/`group_trigger`，但缺“多 bot 群”的现实策略（例如 allowMentionlessInMultiBotGroup）
...
7) **工具层的“权限现实”：不是只有 scope，还包括资源分享/成员关系**  
   ...
   - 启发：如果 X-Claw 未来加飞书 Drive/Docs/Tasks 工具，必须把“资源共享前置条件”写进错误提示/修复建议，否则用户会被权限问题困死

#### (3) 对 X-Claw 的落地映射（我们该补的飞书短板）
...
### 3.16 ✅ UltimateSearchSkill（Shell）— “双引擎搜索 + 证据标准 + 失败降级链” 的可复用技能包（已完成：2026-03-02）
...
对 X-Claw 来说，它提供的是“Web 工具系统的正确打开方式”：**工具 + 策略 + 证据格式**三位一体。

#### (2) 最值得抄的 6 个点（我们可以直接落到工具策略层）

1) **双引擎交叉验证（dual-search）是默认，而不是高级选项**  
   - 关键位置：`ref/UltimateSearchSkill/SKILL.md`（工具选择与证据标准）
   - 启发：事实类结论要求 ≥2 来源；否则明确置信度。这可以变成 X-Claw 的 web tool policy（影响模型行为非常大）
...
3) **Fetch 的三级降级链（Extract → Scrape → error）**  
   ...
   - 启发：X-Claw 的 web fetch 也应提供“可配置的降级链”和“失败原因可解释”（被 403/JS challenge/超时/内容过大）
...
5) **安全基线：服务只绑定 127.0.0.1 + API key + SSH tunnel 管理**  
   ...
   - 启发：如果 X-Claw 未来提供 web proxy / scraping service，一律“本机默认关闭公网”，并给出安全模板

6) **输出规范：结论先行 + 引用格式固定 + 禁止编造引用**  
   - 关键位置：`ref/UltimateSearchSkill/SKILL.md`（引用/置信度/冲突处理）
   - 启发：这部分非常适合直接变成 X-Claw 的“web/search 响应模板”，让用户更信任结果

#### (3) 对 X-Claw 的落地映射（我们怎么“吸收而不变重”）
...
- 生态层（可选）：
  - 把 UltimateSearchSkill 当作“外部 skill 包”示例：X-Claw 的 skills 可以不是纯 prompt，还可以带脚本/compose（但 ref 不进 git，仅参考其结构）

#### (4) 最小 PR 切法（不引入重服务也能提升）

- PR-1：Web 工具“证据标准”提示模板（至少 2 来源 + 冲突处理 + 置信度）
- PR-2：`web_search_dual`（并行双引擎 + 结果去重/聚合）
- PR-3：web fetch 降级链（可配置 extractor，失败原因分类）

---

### 3.17 ✅ nanoclaw（TS）— “容器隔离 + per-group 队列 + scheduler” 的极简安全架构（已完成：2026-03-02）
...
对 X-Claw 的启发很直接：我们可以保持 Go 的轻量，但把“危险工具”做成可选的 sandbox 后端。
...
1) **Per-group queue（会话级队列）+ 全局并发上限**  
   ...
   - 启发：X-Claw 当前 gateway 是单 agent loop 串行消费 inbound；未来要支持多会话长任务，必须引入“会话调度层”
...
2) **Steering 的“硬中断”实现：写 IPC close sentinel（_close）**  
   ...
   - 启发：与 Pi 的“跳过剩余 tool calls”相似，但更硬：直接让容器端 wind down。  
     X-Claw 可以先做软中断（跳过后续 tool），再做硬中断（取消 exec/web 请求）
...
4) **任务容器的“快速关闭”策略（避免 idle timeout 浪费资源）**  
   ...
   - 启发：X-Claw 的 cron/后台任务也应在完成后“自动收尾”：释放并发槽位、写 run log、推送通知
...
6) **SQLite 作为“唯一状态源”（messages/groups/sessions/tasks）**  
   ...
   - 启发：X-Claw 已经有 workspace/session/task ledger，把这些信息统一落到 SQLite（Phase B/C）会更可靠（并发、查询、迁移都更好）

7) **“不加功能，加 skills” 的治理策略**  
   - README：Contributing 部分（skills-first）
   - 启发：X-Claw 也可以把“渠道/重能力”更多做成 skills 包/插件，而不是核心仓库不断膨胀

#### (3) 对 X-Claw 的落地映射（我们怎么吸收它的安全与调度）
...
- PR-1：`exec` 增加可选 docker sandbox（只挂载 workspace）+ 配置开关（默认关闭）
- PR-2：Gateway inbound 加“会话分桶队列”（先只做串行 per-session + 并发上限）
- PR-3：cron/task run log（JSONL/SQLite）+ 失败重试策略（可配置）

---

### 3.18 ✅ zeroclaw（Rust）— “安全（estop）+ MCP 多传输 + runtime trace” 的 pico-scale 竞品工程答案（已完成：2026-03-02）
...
对 X-Claw 来说，它提供了一个很好的参照系：我们 Roadmap 的 5 个差异化支柱在它那里都有可落地的工程形态。
...
1) **MCP Transport 抽象：stdio / HTTP / SSE 三种传输统一到一个 trait**  
   ...
   - 启发：X-Claw 的 MCP Bridge（Phase D）如果想“真的能用”，必须把这些边角考虑进去
...
3) **estop：kill switch 不是布尔值，而是可组合的安全层级**  
   ...
   - 支持的级别：
     ...
     - frozen_tools（冻结工具）
   - 启发：这非常适合 X-Claw 的“Policy-first Tools”支柱：把风险控制做成一等能力
...
5) **OTP gating：resume 操作可要求 OTP（避免误操作/越权）**  
   ...
   - 启发：如果 X-Claw 将来暴露公网 API，必须有类似“危险操作二次确认”的设计（CLI 用确认，Gateway 用 token/OTP）
...
8) **项目内 RFC/Project docs：把复杂机制拆解并可审查**  
   ...
   - 启发：X-Claw 的长期演进也需要这种“可审计的设计文档”，否则功能一多就不可控

#### (3) 对 X-Claw 的落地映射（直接对齐我们的 Roadmap 支柱）
...
对 X-Claw 来说，虽然我们不一定走“多服务 + UI + 容器沙箱”全家桶路线，但它提供了非常清晰的对照：  
...
1) **“多进程/多服务职责分离”非常清楚：Backend / Executor-Manager / Executor / Frontend**  
   ...
   - X-Claw 启发：即便我们仍是单二进制，也可以在内部抽象出同样的边界：  
...
2) **Callback 模型：每次 agent 输出/状态变化都回传，UI 只做展示，不做推理**  
   ...
   - 启发：X-Claw 的 Gateway 未来如果做轻量 UI（或 WebSocket 推送），也应该是“事件流”而不是“轮询拼 JSON”
...
5) **Slash Commands 必须“原样透传”，否则 SDK 无法识别**  
   ...
   - 启发：对 X-Claw 来说，如果我们未来加 slash 指令体系，必须把“命令解析”做成稳定协议（而不是靠 LLM 猜）
