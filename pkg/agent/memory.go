...
func NewMemoryStoreAt(memoryDir string) *MemoryStore {
	...
	if err := os.MkdirAll(memoryDir, 0o755); err != nil {
		initErr = fmt.Errorf("failed to initialize memory directory %q: %w", memoryDir, err)
		logger.WarnCF("agent", "Memory store initialization failed", map[string]any{
...
func (ms *MemoryStore) ensureReady() error {
	if ms == nil {
