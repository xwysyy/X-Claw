...
func TestLoadOpenClawConfig(t *testing.T) {
	...
	if workspace != "~/.x-claw/workspace" {
		t.Errorf("expected workspace '~/.x-claw/workspace', got '%s'", workspace)
	}
...
func TestConvertToXClaw(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, warnings, err := cfg.ConvertToXClaw("")
	if err != nil {
	...
	if picoCfg.Agents.Defaults.Workspace != "~/.x-claw/workspace" {
		t.Errorf("expected workspace '~/.x-claw/workspace', got '%s'", picoCfg.Agents.Defaults.Workspace)
	}
...
func TestConvertToXClawWithQQAndDingTalk(t *testing.T) {
	tmpDir := t.TempDir()
	configPath := filepath.Join(tmpDir, "openclaw.json")
	...
	picoCfg, _, err := cfg.ConvertToXClaw("")
	if err != nil {
		t.Fatalf("failed to convert config: %v", err)
	}
...
func TestToStandardConfig(t *testing.T) {
	...
	if stdCfg.Agents.Defaults.Workspace != "~/.x-claw/workspace" {
		t.Errorf("expected workspace '~/.x-claw/workspace', got '%s'", stdCfg.Agents.Defaults.Workspace)
	}
