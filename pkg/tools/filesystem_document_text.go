...
func (t *DocumentTextTool) Name() string { return "document_text" }

func (t *DocumentTextTool) ParallelPolicy() ToolParallelPolicy { return ToolParallelReadOnly }
...
func (t *DocumentTextTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	header := fmt.Sprintf("[document_text] path=%s type=%s size=%dB extracted_chars=%d truncated=%v", rawPath, ext, sizeBytes, len([]rune(extracted)), truncated)
...
func extractPDFPlainText(ctx context.Context, path string, maxChars int) (string, bool, error) {
	...
	go func() {
		...
		text, truncated := truncateDocumentRunes(strings.TrimSpace(buf.String()), maxChars)
...
func extractDOCXPlainText(_ context.Context, path string, maxChars int) (string, bool, error) {
	const maxXMLBytes = 12 * 1024 * 1024
	...
	text, more := truncateDocumentRunes(strings.TrimSpace(sb.String()), maxChars)
	if more {
		truncated = true
	}
...
func truncateDocumentRunes(s string, max int) (string, bool) {
	if max <= 0 {
		return s, false
	}
	rs := []rune(s)
